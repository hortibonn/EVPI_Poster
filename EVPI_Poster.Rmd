...
The expected value of perfect information (EVPI) is used to express maximized expected utility. It is the difference between the expected value given perfect information and the best expected value given current alternatives [@Strong2014]. EVPI can help identify critical knowledge gaps [@Coyle2008; @hubbard2014] and reveal variables for which more information would better indicate the best decision outcomes  [@lanzanova2019]. EVPI is often expressed as the highest price that decision makers should be willing to pay for perfect information [@hubbard2014]. However, there are often decision criterion other than money. Therefore, we define EVPI as the potential gain given perfect information on all of the unknown model factors.
...
The expected value of deposit investment $Expdeposit$ is calculated as the the probability of different states of the economy (x-axis) times the expected loss or gain in each condition (y-axis):
$$
